...
/**
 * Universal hook for subscribing to Tarobase data.
 *
 ...
 * @example
 * // For a single entity
 * const { data, loading, error } = useTarobaseData<PostsData | null>(
 *   subscribePost,
 *   true,
 *   postId
 * );
 *
 * // For a collection
 * const { data, loading, error } = useTarobaseData<PostsData[]>(
 *   subscribeManyPosts,
 *   true,
 ...
 * const { data, loading, error } = useTarobaseData<CommentsData | null>(
 *   subscribeComment,
 *   true,
 *   postId,
 *   commentId
 * );
 */
export function useTarobaseData<T>(
  ...
  useEffect(() => {
    ...
    return () => {
      console.log('unsubscribing from useTarobaseData', { unsubscribe });
      if (unsubscribe) {
        unsubscribe();
      }
    };
